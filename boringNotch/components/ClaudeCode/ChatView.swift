...
/// Wraps onScrollGeometryChange for macOS 15+ with a no-op fallback for macOS 14.
...
struct ChatApprovalBar: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            // Deny button
            DenyAllowButton(role: .deny, label: "Deny", fontSize: 13, action: onDeny)
                .opacity(showDenyButton ? 1 : 0)
                .scaleEffect(showDenyButton ? 1 : 0.8)

            // Allow button
            DenyAllowButton(role: .allow, label: "Allow", fontSize: 13, action: onApprove)
